<?php
$main_page = "Reports";
$page = "Statutory Compliance MIS Report Details";
$excel_file_name = "Statutory_MIS";
                        ...
                        <div class="col-xs-12">
                            <div class="box">
                                ...
                                <div class="box-body">
                                    ...
                                    if ($error_msg != "") {
                                    header("location:report_mis.php?error=$error_msg");
                                    } else {
                                    if (($activity_id != "")) {
                                    ...
                                    <div style="height: 500px; overflow-x: scroll; overflow-y: scroll;">
                                        <table id="tblData" class="table table-bordered table-striped" border="1">
                                            <thead>
                                                <tr>
                                                    <th colspan="17" style="font-weight: bold; text-align: center;">
                                                        MIS report - Status Report on Statutory Compliance 
                                                        <?php
                                            ...
                                            <tbody>
                                                ...
                                                $mis_status_reportSQL .= "SELECT b.activity, c.purpose, e.short_legislation as legislation, f.department, g.location, SUM(a.budgeted_cost) budgeted_cost, SUM(a.actual_value_covered) ";
                                                $mis_status_reportSQL .= "actual_value_covered, SUM(a.actual_cost) actual_cost, COUNT(*) total_cmp, COUNT(CASE WHEN a.status = -1 AND a.update_status = 1 THEN 1 END) AS ";
                                                ...
                                                $mis_status_reportSQL .= "task_comp_delay, COUNT(CASE WHEN a.status = 0 AND a.due_date>=NOW() THEN 1 END) AS task_not_comp_due FROM risk_management a INNER JOIN mas_activity b ON a.activity_id=b.id INNER JOIN mas_purpose c ON a.purpose_id=c.id ";
                                                $mis_status_reportSQL .= "INNER JOIN mas_legislation e ON a.legislation_id=e.id INNER JOIN mas_department f ON a.department_id=f.id ";
                                                $mis_status_reportSQL .= "INNER JOIN mas_location g ON a.location_id=g.id WHERE a.company_id='$login_company_id' " . $where_clause . " GROUP BY b.activity, c.purpose, g.location, e.legislation, f.department";
                                                ...
                                                $legislation = $fetch_mis_status_reports->legislation;
                                                $activity = $fetch_mis_status_reports->activity;
                                                $purpose = $fetch_mis_status_reports->purpose;
                                                $location = $fetch_mis_status_reports->location;
                                                $budgeted_cost = $fetch_mis_status_reports->budgeted_cost;
                                                $actual_cost = $fetch_mis_status_reports->actual_cost;
                                                $actual_value_covered = $fetch_mis_status_reports->actual_value_covered;
